...
final class SystemProxyManager: @unchecked Sendable {
    ...
    func snapshotProxyService(_ service: String) async -> ProxySnapshot {
        ...
        return ProxySnapshot(web: web, secure: secure, auto: auto, autoState: autoState)
    }

    func setSystemProxy(host: String, port: Int) async throws {
        ...
        let pacURL = try writePacFile(host: host, port: port)
    ...
    private func writePacFile(host: String, port: Int) throws -> URL {
        let proxyHost = host == "0.0.0.0" || host.isEmpty ? "127.0.0.1" : host
        let proxy = "PROXY \(proxyHost):\(port); DIRECT"
        let pac = """
        // Generated by FigmaCN. Only Figma traffic is routed through the local MITM proxy.
