...
PAC 只把 `figma.com` 和 `*.figma.com` 路由到本地 mitmdump，其他域名返回 `DIRECT`，避免 GitHub、VS Code、浏览器等非 Figma 流量被本 App 接管。
...
- `validate_lang.py`：校验 `zh.json` 的结构是否合理
- `lang/manifest.json`：记录语言包来源、key 数量和合并统计
- `lang/zh.json`：主 Figma app 中文包
- `lang/auth-zh.json`：登录、账号、鉴权相关中文包
- `lang/prototype_app_beta-zh.json`：原型预览相关中文包
- `lang/*_latest.en.json`：捕获到的英文源包，作为翻译和对齐依据
...
```text
figma_app...min.en.json(.br)          -> Runtime/lang/zh.json
auth...min.en.json(.br)               -> Runtime/lang/auth-zh.json
auth_iframe...min.en.json(.br)        -> Runtime/lang/auth-zh.json
prototype_app...min.en.json(.br)      -> Runtime/lang/prototype_app_beta-zh.json
prototype_app_beta...min.en.json(.br) -> Runtime/lang/prototype_app_beta-zh.json
community...min.en.json(.br)          -> Runtime/lang/community-zh.json
```

`community` 规则已经预留，但仓库当前还没有 `community-zh.json`。捕获到对应英文包并生成中文包后即可生效。
...
4. 拷贝 `Runtime/injector.py`、校验脚本、启动脚本和 `Runtime/lang/*.json`
