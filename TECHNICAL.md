...
```text
Figma
  -> 显式启动参数 --proxy-server=http://127.0.0.1:<port>
  -> 本地 mitmdump
  -> Runtime/injector.py
...
系统 PAC 仍会被启用，作为系统层备用代理路径。PAC 只把 `figma.com` 和 `*.figma.com` 路由到本地 mitmdump。非 Figma 域名不会走本地 `8080`：
...
12. 退出当前 Figma 进程。
13. 清理 Figma 资源缓存目录。
14. 使用 `--proxy-server=http://127.0.0.1:<port>` 显式重新启动 Figma。

如果第 8 步失败，产品不会继续修改系统代理。

第 14 步是当前桌面端可靠生效的关键路径。实测 Figma Desktop 的 Electron 网络进程可能绕过 macOS PAC，直接连接外部 `443`；显式传入 `--proxy-server` 后，Figma 网络进程会连接本地 mitmdump。
...
```bash
open -a Figma --args --proxy-server=http://127.0.0.1:8080 --proxy-bypass-list="<-loopback>"
```
...
如果启动时检测到系统代理仍指向 FigmaCN 的 PAC，并且这个备份文件存在，新一轮启动会复用该备份作为恢复基准，而不是先恢复再重新接管。这样可以避免旧版异常退出或授权超时后，下一次开启汉化出现连续两次管理员授权。
